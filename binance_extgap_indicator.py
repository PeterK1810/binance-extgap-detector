...
class TelegramExtGapNotifier:
    ...
    def from_env(cls, timeframe: str) -> Optional["TelegramExtGapNotifier"]:
        ...
        """
        # Try timeframe-specific tokens first, then DETECTOR, then generic
        timeframe_upper = timeframe.upper()  # "1M", "5M", "15M", "1H"
        bot_token = (
            os.getenv(f"TELEGRAM_BOT_TOKEN_EXTGAP_{timeframe_upper}")
            or os.getenv("TELEGRAM_BOT_TOKEN_EXTGAP_DETECTOR")  # Fallback for 1M
            or os.getenv("TELEGRAM_BOT_TOKEN")
        )
        chat_ids_str = (
            os.getenv(f"TELEGRAM_CHAT_IDS_EXTGAP_{timeframe_upper}")
            or os.getenv("TELEGRAM_CHAT_IDS_EXTGAP_DETECTOR")  # Fallback for 1M
