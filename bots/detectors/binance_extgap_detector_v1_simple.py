#!/usr/bin/env python3
"""
Binance External Gap Detector - Version 1 (Simple Reset Logic)
================================================================

Pure gap detection without trade simulation.
Reset logic: After any gap → bearish_candidate=low, bullish_candidate=high
...
class GapDetectorState:
    ...
    Version 1: Simple reset logic
    - After any gap: bearish_candidate = candle.low, bullish_candidate = candle.high
    """
    ...
    def add_candle(self, candle: Candle) -> Optional[ExternalGap]:
        ...
        # RESET CANDIDATES (V1 Simple Logic)
        self.bearish_candidate_low = candle.low
        self.bearish_candidate_idx = candle.open_time
        self.bullish_candidate_high = candle.high
        self.bullish_candidate_idx = candle.open_time
...
class TelegramNotifier:
    ...
    async def notify_initial_trend(self, gap: ExternalGap):
        ...
        emoji = "🟢" if gap.polarity == "bullish" else "🔴"
        trend_arrow = "⬆️" if gap.polarity == "bullish" else "⬇️"
        text = (
            f"📈 <b>TENDANCE INITIALE DÉTECTÉE - {gap.symbol}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S')} UTC\n"
            f"💡 Tendance: {emoji} <b>{gap.polarity.upper()}</b> {trend_arrow}\n"
            f"💰 Niveau du gap: <b>{gap.gap_level:,.2f} USDT</b>\n"
            f"🕒 Détecté à: {gap.detection_bar_time.strftime('%H:%M:%S')}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"✅ Le bot surveille maintenant les inversions de tendance"
        )
        await self.send_message(text)
...
class BinanceExtGapDetector:
    ...
    async def _handle_candle(self, candle: Candle):
        ...
        if gap:
            logging.info(f"🚨 GAP DETECTED: {gap.polarity.upper()} at {gap.gap_level:.2f} | First:{gap.is_first_gap} Reversal:{gap.is_reversal}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Binance External Gap Detector - Version 1 (Simple Reset Logic)"
    )
    parser.add_argument(
        "--symbol",
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="CSV output path (default: data/extgap_detector_v1_gaps.csv)"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
...
async def main():
    ...
    logging.info("=" * 80)
    logging.info("Binance External Gap Detector - Version 1 (Simple Reset Logic)")
    logging.info("=" * 80)
    ...
    if telegram:
        ...
        await telegram.notify_startup(args.symbol, args.timeframe, version="v1")
    else:
