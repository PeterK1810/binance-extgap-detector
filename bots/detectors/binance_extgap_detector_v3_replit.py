#!/usr/bin/env python3
"""
Binance External Gap Detector - Version 3 (Replit Edition)
============================================================

Replit-compatible gap detector with CLI argument parser.
...
if parent_env.exists():
    ...
    # Fallback: try current directory (Replit may place .env here)
    local_env = Path(__file__).parent / ".env"
    if local_env.exists():
        load_dotenv(local_env)
    else:
        logging.info("No .env file found - using environment variables from system (Replit Secrets)")
...
def parse_args() -> dict:
    """
    Parse command-line arguments for Replit deployment.

    Returns:
        dict: Configuration dictionary with symbol, timeframe, stats_interval, etc.
    """
    parser = argparse.ArgumentParser(
        description="Binance External Gap Detector V3 - PineScript Algorithm (Replit Edition)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Default configuration (BTCUSDT, 2m, 1h stats)
  python3 binance_extgap_detector_v3_replit.py

  # Custom symbol and timeframe
  python3 binance_extgap_detector_v3_replit.py --symbol ETHUSDT --timeframe 5m

  # Change stats interval
  python3 binance_extgap_detector_v3_replit.py --stats-interval 30m

  # Debug logging
  python3 binance_extgap_detector_v3_replit.py --log-level DEBUG
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="BTCUSDT",
        help="Trading symbol (default: BTCUSDT). Examples: ETHUSDT, SOLUSDT, BNBUSDT"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="2m",
        help="Timeframe for gap detection (default: 2m). Examples: 1m, 2m, 3m, 5m, 15m, 1h"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1h",
        help="Statistics notification interval (default: 1h). Examples: 10m, 30m, 1h, 2h, 4h"
    )
    ...
    # Validate and parse intervals
    try:
        timeframe_minutes = parse_interval_minutes(args.timeframe)
        stats_interval_minutes = parse_interval_minutes(args.stats_interval)
    ...
    print()
    print("=" * 70)
    print("  Binance External Gap Detector v3 - Replit Edition")
    print("  PineScript '[THE] eG v2' Algorithm")
    print("=" * 70)
    print(f"  Symbol:          {args.symbol.upper()}")
    print(f"  Timeframe:       {args.timeframe} ({timeframe_minutes} minutes)")
    print(f"  Stats interval:  {args.stats_interval} ({stats_interval_minutes} minutes)")
    ...
    return {
        ...
        "stats_interval": args.stats_interval.lower(),
...
class TelegramNotifier:
    ...
    async def notify_reversal(self, gap: ExternalGap, prev_polarity: str, prev_sequence: int, prev_level: float,
        ...
        profit_section = ""
        if trade_result:
            pnl_emoji = "✅" if trade_result.gross_pnl > 0 else "❌"
            ...
            profit_section = (
                f"\n💰 <b>P&L Position Fermée:</b>\n"
                f"  {pnl_emoji} {trade_result.closed_side}: "
                f"<b>{trade_result.gross_pnl:+.2f} USD ({trade_result.pnl_percent:+.2f}%)</b>\n"
                f"  📊 Entrée: {trade_result.entry_price:,.2f} → Sortie: {trade_result.exit_price:,.2f}\n"
                f"  🔢 Quantité: {trade_result.quantity:.6f} {gap.symbol.replace('USDT', '')}\n"
                f"\n"
            )
        ...
        text = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ {gap.detection_bar_time.strftime('%H:%M:%S')} UTC\n"
            f"{old_emoji} <b>{prev_polarity.upper()} #{prev_sequence}</b> → "
...
class BinanceExtGapDetector:
    ...
    async def _handle_candle(self, candle: Candle):
        ...
        if not is_candle_aligned(candle.open_time, self.timeframe_minutes):
            logging.error(f"❌ MISALIGNED CANDLE: {candle.open_time} (expected alignment to {self.timeframe_minutes}m)")
        ...
        if self.last_candle_time is not None:
            ...
            if candle.open_time > expected_next:
                ...
                logging.warning(f"⚠️ GAP IN DATA: {self.last_candle_time} → {candle.open_time} ({missing_count} candles missing)")
        ...
        if gap:
            logging.info(f"🚨 GAP DETECTED: {gap.polarity.upper()} #{gap.sequence_number} at {gap.gap_level:.2f} | "
                                ...
                                if is_closed:
                                    ...
                                    if not self.first_aligned_candle_received:
                                        if is_candle_aligned(candle.open_time, self.timeframe_minutes):
                                            logging.info(f"✅ First aligned candle received: {candle.open_time}")
                                            self.first_aligned_candle_received = True
                                        else:
                                            logging.warning(f"⚠️ Skipping misaligned candle: {candle.open_time} "
...
async def main():
    ...
    # Parse command-line arguments (Replit-compatible)
    config = parse_args()
    ...
    logging.info("=" * 80)
    logging.info("Binance External Gap Detector - Version 3 (Replit Edition)")
    logging.info("=" * 80)
