...
Usage:
    python paradex_extgap/binance_extgap_indicator_15m.py --symbol BTCUSDT --timeframe 15m
"""
...
# Timeframe in minutes for 15m indicator
...
class TelegramExtGapNotifier:
    ...
    def from_env(cls, timeframe: str) -> Optional["TelegramExtGapNotifier"]:
        ...
        """
        # Use 15M credentials for 15M timeframe
        bot_token = (
            os.getenv("TELEGRAM_BOT_TOKEN_EXTGAP_15M")
            or os.getenv("TELEGRAM_BOT_TOKEN")
        )
        chat_ids_str = (
            os.getenv("TELEGRAM_CHAT_IDS_EXTGAP_15M")
    ...
    async def notify_status(self, status: str, reason: str = "", symbol: str = "BTCUSDT", stats_interval: str = "1h") -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="data/indicators/gaps/binance_extgap_15m_gaps.csv",
        help="Output CSV file for gaps (default: data/indicators/gaps/binance_extgap_15m_gaps.csv)",
    )

    parser.add_argument(
        "--trades-output",
        type=str,
        default="data/indicators/trades/binance_extgap_15m_trades.csv",
        help="Output CSV file for trades (default: data/indicators/trades/binance_extgap_15m_trades.csv)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Statistics notification interval (e.g., 1h, 2h, 4h). Defaults to STATS_INTERVAL_EXTGAP_15M env var or 1h",
    )
...
async def main() -> None:
    ...
    if stats_interval is None:
        ...
        stats_interval = os.getenv(f"STATS_INTERVAL_EXTGAP_{tf_upper}", "1h")
