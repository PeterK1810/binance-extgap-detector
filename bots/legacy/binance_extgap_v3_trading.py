...
def prompt_configuration() -> dict:
    ...
    print()
    print("⏰ TIMEFRAME")
    print("   Binance Futures supported: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d")
    print("   ⚠️  Note: 2m is NOT supported by Binance Futures")
    timeframe = input("   Enter timeframe [5m]: ").strip().lower() or "5m"

    # Validate timeframe
    valid_timeframes = ['1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d', '3d', '1w', '1M']
    if timeframe not in valid_timeframes:
        print(f"\n❌ Error: '{timeframe}' is not supported by Binance Futures")
        print(f"   Valid timeframes: {', '.join(valid_timeframes)}")
        sys.exit(1)

    # Stats notification interval
    print()
    print("📈 STATISTICS NOTIFICATION INTERVAL")
...
class TelegramNotifier:
    ...
    async def notify_startup(self, symbol: str, timeframe: str, stats_interval: str):
        ...
        text = (
            f"🚀 <b>BOT V3 DÉMARRÉ - {symbol}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S')} UTC\n"
            f"📊 Version: <b>V3 (PineScript Algorithm)</b>\n"
            f"⏱️ Timeframe: <b>{timeframe}</b>\n"
