...
class TelegramNotifier:
    ...
    async def notify_gap_detected(self, symbol: str, polarity: str, gap_level: float, sequence: int):
        ...
        emoji = "🟢" if polarity == "bullish" else "🔴"
        arrow = "⬆️" if polarity == "bullish" else "⬇️"
        text = (
            f"{emoji} <b>GAP DETECTED - {symbol}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S')} UTC\n"
            f"📊 Signal: <b>{polarity.upper()} #{sequence}</b> {arrow}\n"
            f"💰 Gap Level: <b>{gap_level:,.4f}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔔 Tracking candles until reversal..."
        )
        await self.send_message(text)

    async def notify_reversal(self, symbol: str, old_polarity: str, new_polarity: str, gap_level: float):
        """Send reversal notification."""
        old_emoji = "🔴" if old_polarity == "bearish" else "🟢"
        new_emoji = "🟢" if new_polarity == "bullish" else "🔴"
        text = (
            f"🔄 <b>REVERSAL - {symbol}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S')} UTC\n"
            f"{old_emoji} {old_polarity.upper()} → {new_emoji} <b>{new_polarity.upper()}</b>\n"
            f"💰 New Gap Level: <b>{gap_level:,.4f}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        await self.send_message(text)
    ...
    async def notify_candle_result(
        ...
        candle_emoji = "🟩" if candle_close > candle_open else ("🟥" if candle_close < candle_open else "⬜")
        signal_emoji = "⬆️" if polarity == "bullish" else "⬇️"

        streak_text = ""
        if stats.current_win_streak > 0:
            streak_text = f"🔥 Win Streak: {stats.current_win_streak}"
        elif stats.current_lose_streak > 0:
            streak_text = f"💧 Lose Streak: {stats.current_lose_streak}"

        text = (
            f"{result_emoji} <b>{result_text} - {symbol}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S')} UTC\n"
            f"📊 Signal: {polarity.upper()} {signal_emoji}\n"
            f"{candle_emoji} O: {candle_open:,.4f} → C: {candle_close:,.4f}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📈 W: {stats.wins} | L: {stats.losses} | N: {stats.nulls}\n"
    ...
    async def notify_stats_summary(
        ...
        for symbol, stats in symbol_stats.items():
            ...
            if stats.signal_active:
                signal_indicator = "🟢" if stats.current_signal_polarity == "bullish" else "🔴"
        ...
        for symbol, stats in symbol_stats.items():
            ...
            streak = ""
            if stats.current_win_streak > 0:
                streak = f"🔥{stats.current_win_streak}W"
            elif stats.current_lose_streak > 0:
                streak = f"💧{stats.current_lose_streak}L"
        ...
        if aggregated.current_win_streak > 0:
            agg_streak = f"🔥 Current: {aggregated.current_win_streak} wins"
        elif aggregated.current_lose_streak > 0:
            agg_streak = f"💧 Current: {aggregated.current_lose_streak} losses"

        text = (
            f"📊 <b>15M STATS SUMMARY</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
