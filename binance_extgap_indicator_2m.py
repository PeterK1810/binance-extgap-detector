...
class TelegramExtGapNotifier:
    ...
    def __init__(self, bot_token: str, chat_ids: List[str], instance_id: str):
    ...
    @classmethod
    def from_env(cls) -> Optional["TelegramExtGapNotifier"]:
        """Create notifier from environment variables.

        Checks for timeframe-specific variables first, falls back to generic.
        - TELEGRAM_BOT_TOKEN_EXTGAP_2M or TELEGRAM_BOT_TOKEN
        - TELEGRAM_CHAT_IDS_EXTGAP_2M or TELEGRAM_CHAT_IDS
        - INSTANCE_ID (defaults to "LOCAL")

        Returns:
            TelegramExtGapNotifier instance or None if credentials missing
        """
        # Try timeframe-specific tokens first
        bot_token = os.getenv("TELEGRAM_BOT_TOKEN_EXTGAP_2M") or os.getenv(
            "TELEGRAM_BOT_TOKEN"
        )
        chat_ids_str = os.getenv("TELEGRAM_CHAT_IDS_EXTGAP_2M") or os.getenv(
            "TELEGRAM_CHAT_IDS"
        )
        ...
        return cls(bot_token, chat_ids, instance_id)
    ...
    async def notify_status(self, status: str, reason: str = "", symbol: str = "BTCUSDT", timeframe: str = "2m") -> None:
        ...
        if status == "started":
            from datetime import datetime, timezone
            message = (
                f"🚀 <b>BOT INDICATOR 2M DÉMARRÉ - {symbol}</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S')} UTC\n"
                f"📊 Version: <b>2M Trading Bot</b>\n"
    ...
    async def notify_trade_open(self, trade: ExtGapTrade, gap_level: float, sequence_number: int = 1) -> None:
        ...
        polarity = "BULLISH" if trade.side == "long" else "BEARISH"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="2m",
        help="Kline interval (default: 2m)",
    )

    parser.add_argument(
        "--output",
        type=str,
        default="data/binance_extgap_2m_gaps.csv",
        help="Output CSV file for gaps (default: data/binance_extgap_2m_gaps.csv)",
    )

    parser.add_argument(
        "--trades-output",
        type=str,
        default="data/binance_extgap_2m_trades.csv",
        help="Output CSV file for trades (default: data/binance_extgap_2m_trades.csv)",
    )
...
def setup_logging(log_level: str) -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            stdout_handler,
            logging.FileHandler("binance_extgap_indicator_2m.log", encoding='utf-8'),
        ],
    )

    # CRITICAL: Completely disable Telegram/HTTP library logs to prevent emoji encoding crashes on Windows
    # These libraries log messages containing emojis which cause UnicodeEncodeError with Windows cp1252
...
async def main() -> None:
    ...
    notifier = TelegramExtGapNotifier.from_env()
